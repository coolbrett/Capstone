...
function generateNodesAndLinks(filePath){
    if (typeof filePath != 'string'){
        ...
        for (let i = 0; i < movies.length; i++){
            let temp = {};
            //Changed values here
            temp['id'] = movies[i]['Title'];
            temp['rank'] = movies[i]['Rank'];
            nodes.push(temp);
            for (let j = 0; j < movies[i]['Actors'].length; j++){
                let actor = {};
                actor['id'] = movies[i]['Actors'][j]
                nodes.push(actor);

                let link = {};
                link['source'] = movies[i]['Title'];
                link['target'] = actor['id'];
                links.push(link);
            }
        }
        fs.writeFile('./Data/nodesInfo2.json', JSON.stringify(container, null, 1), (error) => {
            if (error) throw error;
        });
